...
const ProjectDetail: React.FC<ProjectDetailProps> = ({ projectId }) => {
  ...
  const toggleWorkdayMutation = useMutation({
    mutationFn: (date: Date) => {
      ...
      const existingWorkday = workdays.find((w) => {
        const workdayDate = new Date(w.date);
        return (
          workdayDate.getUTCFullYear() === date.getUTCFullYear() &&
          workdayDate.getUTCMonth() === date.getUTCMonth() &&
          workdayDate.getUTCDate() === date.getUTCDate()
        );
      });
    ...
    onError: (error: any) => {
      const message =
        error.message === "Workday already exists for this date"
          ? "This date has already been added as a workday."
          : `Failed to update workday: ${error.message}`;
      toast({
        title: "Error",
        description: message,
        variant: "destructive",
    ...
    onSuccess: (response: any) => {
      if (response && response.id) {
        ...
        toast({
          title: "Success",
          description:
            "Invoice generated successfully. Please check the Invoices page.",
          variant: "default",
        });
        queryClient.invalidateQueries({ queryKey: ["/api/invoices"] });
        navigate("/invoices");
  ...
  useEffect(() => {
    if (workdays.length > 0) {
      ...
      setSelectedDays([]); // Ensure selectedDays is empty if there are no workdays
    }
  }, [workdays]);
  ...
  useEffect(() => {
    ...
    for (let i = 0; i < firstDayOfWeek; i++) {
      const prevMonthDay = new Date(
        Date.UTC(year, month, -firstDayOfWeek + i + 1),
      );
      daysArray.push({
        date: prevMonthDay,
        day: prevMonthDay.getUTCDate(),
        isCurrentMonth: false,
        isSelected: selectedDays.some(
          (d) =>
            d.getUTCFullYear() === prevMonthDay.getUTCFullYear() &&
            d.getUTCMonth() === prevMonthDay.getUTCMonth() &&
            d.getUTCDate() === prevMonthDay.getUTCDate(),
        ),
        isDisabled: true,
      });
    }
    ...
    for (let i = 1; i <= daysToAdd; i++) {
      ...
      daysArray.push({
        date: nextMonthDay,
        day: nextMonthDay.getUTCDate(),
        isCurrentMonth: false,
        isSelected: selectedDays.some(
          (d) =>
            d.getUTCFullYear() === nextMonthDay.getUTCFullYear() &&
            d.getUTCMonth() === nextMonthDay.getUTCMonth() &&
            d.getUTCDate() === nextMonthDay.getUTCDate(),
        ),
        isDisabled: true,
      });
    }
  ...
  const handleGenerateInvoice = () => {
    ...
    if (project.type === "daily_rate" && selectedDays.length === 0) {
      toast({
        ...
        description:
          "Please select at least one workday to generate an invoice",
        variant: "destructive",
      });
      return;
    }
    ...
    const invoiceData: CreateInvoiceInput = {
      ...
      workdaysIds: workdays
        .filter((w) =>
          selectedDays.some(
            (d) =>
              d.getUTCFullYear() === new Date(w.date).getUTCFullYear() &&
              d.getUTCMonth() === new Date(w.date).getUTCMonth() &&
              d.getUTCDate() === new Date(w.date).getUTCDate(),
          ),
        )
        .map((w) => w.id),
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          ...
          <div>
            ...
            <div className="mb-4">
              ...
              <div className="text-white">{client.billingEmail}</div>
            </div>
            <div>
              <label className="block text-sm font-medium text-muted-foreground mb-1">
                Status
        ...
        <Card className="bg-background-paper shadow-lg p-6">
            ...
            <div className="flex flex-col md:flex-row md:items-center md:justify-between">
              ...
              <div className="mt-4 md:mt-0">
                <div className="flex items-center bg-background rounded-md p-3 text-white font-medium">
                  Total:{" "}
                  <span className="ml-1 text-primary text-lg">
          ...
          <div className="mt-6 flex justify-end">
            <Button
              onClick={handleGenerateInvoice}
              disabled={
                generateInvoiceMutation.isPending || selectedDays.length === 0
              }
              className="flex items-center"
            >
              {generateInvoiceMutation.isPending ? (
          ...
          <div className="flex flex-col md:flex-row md:items-center md:justify-between">
            <div>
              ...
              <p className="text-muted-foreground">
                Generate an invoice for the full amount of{" "}
